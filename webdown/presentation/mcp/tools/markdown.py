...
def _usage_guide_text() -> str:
    ...
    return (
        ...
        "  explore_sitemap(base_url, max_pages=1000)\n"
        "    Discover all pages from a website sitemap. Call this FIRST before converting.\n"
        "    Returns page URLs with metadata (lastmod, changefreq, priority), total_available,\n"
        "    and a truncated flag (True if max_pages capped the results). Default is 1000;\n"
        "    set max_pages=0 for all pages.\n\n"
        ...
        "  convert_all_pages(base_url, max_pages=1000, whitelist, blacklist, resume=False,\n"
        "                     capture_artifacts=False)\n"
        "    Convert EVERY page from a sitemap into one combined Markdown file.\n"
        "    Failing pages are isolated (job ends completed_with_errors; successful pages are\n"
        "    saved). Set resume=True to skip pages already converted successfully for this\n"
        "    site. Set capture_artifacts=True to save crash HTML+traceback for debugging.\n"
        "    max_pages defaults to 1000; set 0 for all. Track with get_job_progress, then\n"
        "    save to disk with save_markdown_to_file (or download_markdown for small jobs).\n\n"
        ...
        "    Check conversion progress. Returns status, total_pages, processed_pages,\n"
        "    failed_pages, total_available, truncated, and error_message.\n"
        "    Status can be: processing, completed, completed_with_errors, or failed.\n\n"
        ...
        "    Set split_per_page=True to write one .md per successful page into output_path.\n\n"
        "  list_markdown_files()\n"
        ...
        "    Check total_available and truncated to know if results were capped.\n"
        "  • Use convert_single_page() for single pages, convert_all_pages() for whole sites.\n"
        "  • For large conversions, use save_markdown_to_file() instead of download_markdown()\n"
        "    to avoid shipping multi-MB content through the response.\n"
        "  • If a bulk conversion ends completed_with_errors, re-run with resume=True\n"
        "    to fill only the gaps (and set capture_artifacts=True to debug failures).\n"
...
def register_markdown_tools(server: object) -> None:
    ...
    @server.tool(
        description=(
            ...
            "already converted successfully for this site. Set capture_artifacts=True to save "
            "crash HTML + traceback to data/debug/ for any pages that fail conversion. "
            "max_pages defaults to 1000; set to 0 for all pages. "
    ...
    @server.tool(
        description=(
            ...
            "Returns status (processing/completed/completed_with_errors/failed), total_pages, "
            "processed_pages, failed_pages, total_available, truncated, and error_message. "
            "Poll this after starting a conversion."
        ),
    )
