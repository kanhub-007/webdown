...
def _usage_guide_text() -> str:
    ...
    return (
        ...
        "TYPICAL WORKFLOW\n"
        "  1. explore_sitemap(base_url) — see what pages exist on a website\n"
        "  2. convert_single_page(url) — convert one page to Markdown (fast, synchronous)\n"
        "     OR convert_all_pages(base_url) — convert the entire site (for large sites)\n"
        "  3. get_job_progress(job_id) — track conversion progress\n"
        "  4. download_markdown(job_id) — get the generated Markdown content\n"
        "  5. list_markdown_files() — see all previously generated files\n\n"
        "TOOLS REFERENCE\n"
        ...
        "BEST PRACTICES\n"
        "  • Always explore_sitemap() first to see what pages exist.\n"
